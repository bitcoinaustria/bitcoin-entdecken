...
\date{\today}
\institute{}
\titlegraphic{\center\includegraphics[width=0.4\textwidth,keepaspectratio]{title-grafik.jpg}}
...
\newcommand{\misconceptionslide}[4]{%
    % #1: Frame title (e.g., "Umweltauswirkungen")
    % #2: Misconception text (in quotes)
    % #3: Facts list items
    % #4: Fazit text
    \stepcounter{mvnum}
    \subsection{\arabic{mvnum}. #1}
    \begin{frame}<1-2>{M\arabic{mvnum}: #1}
        \begin{columns}[T]
        ...
        \onslide<2->{
            \textcolor{bitcoinorange}{\textbf{Fazit:}} #4
        }
    \end{frame}
}
...
\misconceptionslide{Skalierbarkeit}{Bitcoin kann nur 3-4 Transaktionen pro Sekunde verarbeiten}{%
...
\misconceptionslide{Schneeballsystem}{Bitcoin ist ein Schneeballsystem}{%
...
\misconceptionslide{Spekulationsblase}{Bitcoin ist eine Spekulationsblase ohne fundamentalen Wert}{%
...
\misconceptionslide{Zentralbank-Bedenken}{Bitcoin bedroht die Geldpolitik und wird von Regierungen verboten}{%
