...
\begin{frame}<1-2>{M1: Verbraucht zu viel Energie}
...
\subsection{2. Kein intrinsischer Wert}

\begin{frame}<1-2>{M2: Kein intrinsischer Wert}
...
\begin{frame}<1-2>{M2: Zu volatil für praktische Nutzung}
...
\begin{frame}<1-2>{M3: Regulatorische Unsicherheit}
...
\begin{frame}<1-2>{M4: Technische Limitierungen}
...
\begin{frame}<1-2>{M5: Kriminalitäts-Tool}
...
\begin{frame}<1-2>{M6: Reine Spekulation ohne Wert}
...
\begin{frame}<1-2>{M7: Keine praktische Nutzung}
...
\begin{frame}<1-2>{M8: Bedrohung für Geldpolitik}
...
\begin{frame}<1-2>{M9: Technische Sicherheit}
