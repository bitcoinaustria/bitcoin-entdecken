...
\misconceptionslide[pix/mining.jpg]{Umweltauswirkungen}{Bitcoin-Mining verbraucht zu viel Energie und zerstört die Umwelt}{%
	...
	\item \textbf{190 TWh}/Jahr: rund \textbf{0,6\%} des globalen Stromverbrauchs, unter \textbf{0,1\%} der Emissionen\footnotemark
	\item \textbf{59,4\%} CO$_2$-arm -- 2024 erst \textbf{52,4\%}\footnotemark
	\item \textbf{Wasserkraft} vor Erdgas; Kohle nur \textbf{ca.\ 9\%}\footnotemark
	...
	\only<2->{\footnotetext[1]{Cambridge CCAF, vorläufige Daten für Dezember 2025 (veröffentlicht Juli 2026)}}
	\only<2->{\footnotetext[2]{Stichprobe: gut die Hälfte der globalen Hashrate}}
	\only<2->{\footnotetext[3]{Kohle-Anteil 2022 noch 36,6\%}}
}{Strommix wird rasch sauberer: Verbrauch $+38\%$, Emissionen nur $+20\%$ -- die Emissionsintensität sinkt deutlich.}
...
\begin{frame}[c]{Kein Wert oder zu teuer? Was nun?}
	\begin{center}
		\begin{tikzpicture}
			...
			\node[inner sep=0pt] at (0.35\textwidth,0.192\textwidth) {\includegraphics[width=0.7\textwidth,keepaspectratio]{pix/wert-oder-teuer-20250812_152710.jpg}};
		\end{tikzpicture}
	\end{center}
\end{frame}
...
\misconceptionslide[pix/bitcoin-skalieren-2.jpg]{Skalierbarkeit}{Bitcoin kann nur 3-4 Transaktionen pro Sekunde verarbeiten}{%
	...
	\item \textbf{Lightning Network}: bis zu \textbf{1 Mio.\ TPS} möglich
	\item Visa: \textbf{65.000 TPS}, ca.\ \textbf{15--18 Bio.\ USD}/Jahr\footnotemark
	\item Bitcoin: \textbf{$>$~25 Bio.\ USD} (On-Chain + Layer 2)\footnotemark
	\only<2->{\footnotetext[1]{Visa Inc. Annual Report 2024}}
	\only<2->{\footnotetext[2]{Sygnum Bank Future Finance Report, 2025}}
	\item Lightning ist global, dezentral, ohne Intermediäre
	\item Kein \textbf{Schuldverhältnis}, sondern \textbf{finales Settlement}
...
\misconceptionslide[pix/no-bitcoin-in-shop.png]{Praktische Adoption}{Bitcoin hat keine praktische Anwendung im Alltag}{%
	...
	\only<2->{\footnotetext[1]{Wikipedia: Greshamsches Gesetz}}
...
\begin{frame}{Quellen und weitere Informationen}
	...
	\begin{itemize}
		...
		\item \href{https://forklog.com/en/low-carbon-energy-share-in-bitcoin-mining-exceeds-59/}{Cambridge CCAF 2026: Anteil CO$_2$-armer Energie im Mining über 59\% (vorläufige Daten)}
